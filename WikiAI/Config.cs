...
public record ConfluenceClientConfig(
	string? CONFLUENCE_API_KEY,
	string? CONFLUENCE_DOMAIN,
	string? CONFLUENCE_EMAIL
) : IConfigurable
{
...
public record AzureOpenAIConfig(
	...
	string AOAI_KEY,
	string AOAI_DEPLOYMENT_CHAT,
	string AOAI_DEPLOYMENT_EMBEDDING
);

public record MongoConfig(
	string? MONGO_CONNECTION_STRING,
	string? MONGO_DATABASE_NAME
) : IConfigurable
{
	public bool IsConfigured => !string.IsNullOrWhiteSpace(MONGO_CONNECTION_STRING);
}

public record AISearchConfig(
	string? AISEARCH_ENDPOINT,
	string? AISEARCH_KEY,
	string? AISEARCH_INDEX
) : IConfigurable
{
...
public record PostgresConfig(string? POSTGRES_CONNECTION_STRING) : IConfigurable
{
